...
const {Message, StringSession, Image, Video} = require('./SPIRITSER/');
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.AFPLK == 'spiritser' || config.AFPLK == 'spirit' || config.AFPLK == 'vava' || config.AFPLK == 'spiritser') {
        //thanks to afnanplk
        console.log(
            chalk.green.bold('thanks for watching -key cofirmed-')
        );
         }
         else if (config.AFPLK !== 'spiritser' || config.AFPLK !== 'spirit' || config.AFPLK !== 'vava' || config.AFPLK !== 'spiritser') {
         console.log(
        ...
        console.log(
            chalk.green.bold('spirit ser' + config.WORKTYPE + ' Now 💥'));
          // thanks to afnanplk
    ...
    if (config.SUPPORT == '918281370025-1632403322') {     
                        ...
                        try {
                            ...
                            if (config.NOLOG === 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ SPIRIT  SER ~______~*' +
                                    '\n*🌀 Follow this page other wise chance to get erorr: https://chat.whatsapp.com/KDpHHM89WL6395ApKyFrmF*' +
                                    '\n\n*⚠️ ' + error + '*\n'
                                    , MessageType.text);
                            }
                        }
